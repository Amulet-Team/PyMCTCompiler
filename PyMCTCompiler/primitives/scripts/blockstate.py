    ...
    if carry_properties is None:
        return {
            "to_universal": [
                ...
                {"function": "new_properties", "options": {key: val}},
            ],
            "from_universal": {
        ...
        return {
            "to_universal": [
                ...
                {"function": "new_properties", "options": {key: val}},
                {"function": "carry_properties", "options": carry_properties},
            ],
            "from_universal": {
                f"{universal_namespace}:{universal_block_name}": [
                    {"function": "new_block", "options": default_block},
...
def nbt_from_hex(hex_str: str):
    return {"specification": {"snbt": _snbt_spec_from_hex(bytes.fromhex(hex_str))[1]}}


def _snbt_spec_from_hex(
    nbt_bin: bytes, endianness=">", nbt_type: bytes = None
) -> Union[Tuple[str, dict, bytes], None]:
    ...
    if nbt_type is None:
        ...
        if nbt_type == b"\x00":
            ...
            name = struct.unpack(
                f"{endianness}{name_length}s", nbt_bin[3 : 3 + name_length]
            )[0].decode("utf-8")
    ...
    if nbt_type == b"\x00":
        ...
        payload = list(
            struct.unpack(f"{endianness}{array_length}b", nbt_bin[4 : 4 + array_length])
        )
        ...
        payload = struct.unpack(
            f"{endianness}{payload_length}s", nbt_bin[2 : 2 + payload_length]
        )[0].decode("utf-8")
        ...
        for _ in range(payload_length):
            _, nested_obj, nbt_bin = _snbt_spec_from_hex(
                nbt_bin, endianness, payload_nbt_type
            )
        ...
        payload = list(
            struct.unpack(
                f"{endianness}{array_length}i", nbt_bin[4 : 4 + array_length * 4]
            )
        )
        ...
        payload = list(
            struct.unpack(
                f"{endianness}{array_length}l", nbt_bin[4 : 4 + array_length * 8]
            )
        )
...
def _nbt_mapping_from_hex(nbt_bin: bytes):
    if nbt_bin.startswith(b"\x00"):
        ...
        return {"type": "byte", "functions": {"carry_nbt": {}}}
    elif nbt_bin.startswith(b"\x02"):
        return {"type": "short", "functions": {"carry_nbt": {}}}
    elif nbt_bin.startswith(b"\x03"):
        return {"type": "int", "functions": {"carry_nbt": {}}}
    elif nbt_bin.startswith(b"\x04"):
        return {"type": "long", "functions": {"carry_nbt": {}}}
    elif nbt_bin.startswith(b"\x05"):
        return {"type": "float", "functions": {"carry_nbt": {}}}
    elif nbt_bin.startswith(b"\x06"):
        return {"type": "double", "functions": {"carry_nbt": {}}}
    elif nbt_bin.startswith(b"\x07"):
        return {
            "walk_input_nbt": {"type": "byte_array", "functions": {"carry_nbt": {}}}
        }
...
def coral(
    ...
    return {
        ...
        "from_universal": {
            f"{universal_namespace}:{universal_block_name}": [
                {"function": "new_block", "options": "minecraft:tube_coral"},
                {
                    "function": "map_properties",
                    "options": {
                        "coral_type": {
    ...
    if wall:
        return {
            "to_universal": [
                ...
                },
                {
                    ...
                    "options": {"facing": ['"north"', '"south"', '"west"', '"east"']},
                },
            ],
            "from_universal": {
                f"{universal_namespace}:{universal_block_name}": [
        ...
        return {
            "to_universal": [
                ...
                {
                    "function": "new_properties",
                    "options": {"coral_type": material, "dead": dead, "facing": '"up"'},
                },
            ],
            "from_universal": {
                f"{universal_namespace}:{universal_block_name}": [
...
def material_helper(
    ...
    if default:
        fu.insert(0, {"function": "new_block", "options": default})
    if carry_properties is not None:
        tu.append({"function": "carry_properties", "options": carry_properties})
        fu.append({"function": "carry_properties", "options": carry_properties})
    return {
        "to_universal": tu,
        "from_universal": {f"{universal_namespace}:{universal_block_name}": fu},
    }
...
def leaves(
    ...
    return {
        ...
        "from_universal": {
            f"{universal_namespace}:{universal_block_name}": [
                {"function": "new_block", "options": "minecraft:oak_leaves"},
                {
                    "function": "map_properties",
                    "options": {
                        "material": {
...
def wood(
    ...
    return {
        "to_universal": [
            ...
            {
                "function": "new_properties",
                "options": {"material": material, "stripped": stripped},
            },
            {
                "function": "carry_properties",
                "options": {"axis": ['"x"', '"y"', '"z"']},
        ...
        if wall:
            ...
            return {
                "to_universal": [
                    ...
                    {"function": "new_properties", "options": {"facing": '"up"'}},
                ],
                "from_universal": {
                    f"{universal_namespace}:{universal_block_name}": [
                        {"function": "new_block", "options": default_block},
        ...
        if wall:
            ...
            return {
                "to_universal": [
                    ...
                    {"function": "carry_properties", "options": carry_properties_merge},
                ],
                "from_universal": {
                    f"{universal_namespace}:{universal_block_name}": [
                        {"function": "new_block", "options": default_block},
